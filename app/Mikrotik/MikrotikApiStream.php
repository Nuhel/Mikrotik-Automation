...
class MikrotikApiStream
{
    ...
    var $timeout   = 4;     //  Connection attempt timeout and data read timeout
    ...
    public function connect($ip, $login, $password)
    {
        ...
        if(!strlen($ip) || !strlen($login) || !strlen($password)){
            throw new \Exception("Mikrotik Configuration Error");
        }
    ...
    public function run($comm, $arr = array()){
        if(!$this->connected){
            return collect();
        }
        return collect($this->comm($comm,$arr));
    }
